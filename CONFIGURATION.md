...
Picking up a portal with the physgun requires holding **Alt + Shift** (the `+walk` and `+speed` bindings). Rebound movement keys follow those actions. This modifier does not bypass prop protection or replace a confirmed prop hold. All four portal tool panels show the shortcut.

Initial physgun pickup traces through one reciprocal portal pair to a `prop_physics` or `prop_physics_multiplayer`, checking both endpoints' prop feature, pickup permissions and assembly size. An invisible selection handle lets the native physgun retain its full grab distance, wheel, rotation and freeze controls; its temporary weld preserves the original entity, physics bodies and existing constraints. The beam draws separate entrance and exit segments while aiming and holding. Unsupported assemblies or collision meshes are refused. Dedicated-server probes exercised native wheel, rotation, freeze, constraint retention and repeated pickup. Singleplayer re-pickup, normal prop lighting, and beam continuation in both portal and direct views were manually confirmed for initial remote pickup and ordinary held crossing; multiplayer observer checks remain pending.

`PlanTransport` is a preflight for a rigid assembly; `CommitTransport` checks membership/physics identity and synchronously writes poses. It does not guarantee atomic native VPhysics behavior. Assembly sizes, local constraint lengths and local angular velocities are preserved; independent loose props keep the upstream-style scaling route. Conservative OBB aperture and destination AABB checks can block geometrically valid narrow fits. Self/directed physical transfer is excluded; initial remote pickup supports only the physgun path described above.
